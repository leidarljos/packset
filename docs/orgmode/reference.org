...
| =packset search [--workspace WS] [--as-of TS] QUERY= | ranked claims live now, or those live at =TS=: score, kind, id, text |
...
| =due_at=, =review= | the review clock: =stability=, =difficulty=, =reps=, =last=, =interval_s=, =neglected= (sweeps that found it overdue) |
| =forgotten= | on a tombstone the writer made: =neglect= (a forgettable claim never recalled and missed three sweeps) or =the live cap= (the least retrievable claims past the workspace's cap); a forget by hand carries its own reason. Preferences, rules, readings, goals, trust rows and personas are never forgotten by the writer |
